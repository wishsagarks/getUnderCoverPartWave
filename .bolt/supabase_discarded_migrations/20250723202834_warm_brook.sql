/*
  # Reset Database Schema for Guess Who Now

  1. Drop all existing tables
  2. Create new schema for the project:
     - word_packs: Store curated and custom word packs
     - local_game_configs: Store local game configurations
     - local_game_states: Store local game states for persistence
     - user_profiles: Basic user profiles for online features
     - game_sessions: Online multiplayer game sessions
...
CREATE TABLE local_game_configs (
  ...
  spectator_voting boolean NOT NULL DEFAULT false,
  minigames_enabled boolean NOT NULL DEFAULT true,
  observer_mode boolean NOT NULL DEFAULT true,
  discussion_timer boolean NOT NULL DEFAULT true,
  discussion_time_minutes integer NOT NULL DEFAULT 2 CHECK (discussion_time_minutes >= 1 AND discussion_time_minutes <= 5),
  animated_scoreboard boolean NOT NULL DEFAULT true,
...
CREATE TABLE local_game_states (
  ...
  current_phase text NOT NULL DEFAULT 'setup' CHECK (current_phase IN ('setup', 'onboarding', 'role-reveal', 'clue-giving', 'discussion', 'voting', 'elimination', 'round-end', 'game-end')),
  current_player_index integer NOT NULL DEFAULT 0,
  speaking_order jsonb NOT NULL DEFAULT '[]',
  votes jsonb NOT NULL DEFAULT '{}',
  eliminated_players jsonb NOT NULL DEFAULT '[]',
  game_stats jsonb NOT NULL DEFAULT '{"totalClues": 0, "correctGuesses": 0, "mrXWins": 0, "civilianWins": 0}',
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
-- RLS Policies for local_game_configs
CREATE POLICY "Anyone can read local game configs"
  ON local_game_configs
  FOR SELECT
  TO authenticated, anon
  USING (true);

CREATE POLICY "Anyone can create local game configs"
  ON local_game_configs
  FOR INSERT
  TO authenticated, anon
  WITH CHECK (true);

-- RLS Policies for local_game_states
CREATE POLICY "Anyone can read local game states"
  ON local_game_states
  FOR SELECT
  TO authenticated, anon
  USING (true);

CREATE POLICY "Anyone can manage local game states"
  ON local_game_states
  FOR ALL
  TO authenticated, anon
  USING (true)
...
INSERT INTO word_packs (id, title, description, type, difficulty, content, is_public) VALUES
(
  'general',
  'General Pack',
...
CREATE INDEX idx_local_game_configs_created_at ON local_game_configs(created_at);
CREATE INDEX idx_local_game_states_config_id ON local_game_states(config_id);
...
CREATE TRIGGER update_word_packs_updated_at BEFORE UPDATE ON word_packs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_local_game_states_updated_at BEFORE UPDATE ON local_game_states FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
